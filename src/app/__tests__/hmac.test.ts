...
describe('verifyLinearSignature', () => {
  it('returns true for a valid signature over an object body', () => {
    ...
    expect(verifyLinearSignature(body, sign(body, SECRET), SECRET).valid).toBe(true);
  });

  it('returns false when signature does not match', () => {
    const body = { action: 'create' };
    expect(verifyLinearSignature(body, 'a'.repeat(64), SECRET).valid).toBe(false);
  });

  it('returns false when signature is undefined', () => {
    expect(verifyLinearSignature({}, undefined, SECRET).valid).toBe(false);
  });

  it('returns false for malformed hex (odd-length)', () => {
    expect(verifyLinearSignature({}, 'abc', SECRET).valid).toBe(false);
  });
