...
> **⚠️ Correction — read before filming.** The conclusion this episode reaches, that `export const fields` cannot deploy, is **wrong**. The real cause was a double-deploy race in CI: `hs project upload` auto-deploys after building, and the workflow then deployed the *same build* a second time. A component only deploys successfully once, so whichever attempt ran second failed. `fields` deploys fine — it only broke the redundant second deploy. See [episode 15](26-The%20Bisect%20That%20Lied:%20When%20Your%20Test%20Harness%20Is%20Non-Deterministic.md).
>
